...
- `search` -- search backends, public discovery: [Search Configuration](deployment/configuration/config-search.md)
- `rate_limiting` -- global, user, IP, route-level limits: [Rate Limiting Configuration](deployment/configuration/config-rate-limiting.md)
- `api` -- API prefix, docs paths, CORS, proxies, TLS: [API & Server Configuration](deployment/configuration/config-api.md)
- `server` -- bind address, port, workers, reload: [API & Server Configuration](deployment/configuration/config-api.md)
- `email` -- SMTP setup for verification and password reset: [Email Configuration](deployment/configuration/config-email.md)
- `embeds` -- embed limits and URL preview: [Embeds & URL Preview Configuration](deployment/configuration/config-embeds.md)
