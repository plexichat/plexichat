...
async def get_telemetry_stats(
    ...
    try:
        ...
        stats = telemetry.get_endpoint_stats(
            hours=hours, endpoint_filter=endpoint, client_id_filter=cid
        )
    ...
    try:
        ...
        if format == "json":
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=telemetry_{ts.replace(' ', '_').replace(':', '-')}.json"
                },
            )
            ...
            for s in stats:
                writer.writerow(
                    [
                        ...
                        round(s.avg_queries, 1),
                        round(s.avg_query_time_ms, 2),
                        round(s.error_rate, 2),
                    ]
                )
            return Response(
                ...
                media_type="text/csv",
                headers={
                    "Content-Disposition": f"attachment; filename=telemetry_{ts.replace(' ', '_').replace(':', '-')}.csv"
                },
            )
            ...
            for s in stats:
                lines.append(
                    f"{s.endpoint[:50]:<50} {s.method:<8} {s.count:>8} {s.avg_response_time_ms:>8.1f} {s.p95_response_time_ms:>8.1f} {s.avg_queries:>6.1f} {s.avg_query_time_ms:>8.1f} {s.error_rate:>6.1f}"
                )
            return Response(
                content="\n".join(lines),
                media_type="text/plain",
                headers={
                    "Content-Disposition": f"attachment; filename=telemetry_{ts.replace(' ', '_').replace(':', '-')}.txt"
                },
            )
