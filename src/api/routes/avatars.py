...
async def get_user_avatar(user_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to fetch avatar for user {user_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
        ...
        logger.error(f"Unexpected error in get_user_avatar for user {user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
...
async def upload_my_avatar(
    ...
    if not file.content_type or not file.content_type.startswith('image/'):
        logger.warning(f"User {current_user.user_id} attempted to upload non-image file: {file.content_type}")
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        logger.error(f"Failed to read avatar file upload from user {current_user.user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=400,
    ...
    try:
        try:
            ...
            logger.error(f"Failed to upload avatar for user {current_user.user_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
        ...
        try:
            ...
            logger.debug(f"Failed to invalidate cache for user {current_user.user_id} after avatar upload: {e}")
        ...
        logger.error(f"Unexpected error in upload_my_avatar for user {current_user.user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": {"code": 500, "message": str(e)}}
        )
...
async def delete_my_avatar(current_user: TokenInfo = Depends(get_current_user)) -> SuccessResponse:
    """
    ...
    try:
        try:
            ...
            logger.error(f"Failed to delete avatar for user {current_user.user_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
        ...
        try:
            ...
            logger.debug(f"Failed to invalidate cache for user {current_user.user_id} after avatar deletion: {e}")

        return SuccessResponse(success=deleted)
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error in delete_my_avatar for user {current_user.user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": {"code": 500, "message": str(e)}}
        )
...
async def get_server_icon(server_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to fetch icon for server {server_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
        ...
        logger.error(f"Unexpected error in get_server_icon for server {server_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": {"code": 500, "message": str(e)}}
        )
...
async def upload_server_icon(
    ...
    try:
        try:
            ...
            logger.warning(f"User {current_user.user_id} provided invalid server ID for icon upload: {server_id}")
            raise HTTPException(
                status_code=400,
                detail={"error": {"code": 400, "message": "Invalid server ID"}}
            )
        ...
        try:
            ...
            if not is_owner and not has_permission:
                logger.warning(f"User {current_user.user_id} denied MANAGE_SERVER for server {sid}")
                raise HTTPException(
                    status_code=403,
            ...
            logger.error(f"Error checking permissions for server {sid} icon upload: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
                detail={"error": {"code": 500, "message": "Failed to verify permissions"}}
            )

        # Validate file type
        if not file.content_type or not file.content_type.startswith('image/'):
            logger.warning(f"User {current_user.user_id} attempted to upload non-image icon for server {sid}: {file.content_type}")
            raise HTTPException(
                status_code=400,
                detail={"error": {"code": 400, "message": "File must be an image"}}
            )
        ...
        try:
            ...
            logger.error(f"Failed to read icon file upload for server {sid} from user {current_user.user_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=400,
                detail={"error": {"code": 400, "message": f"Failed to read file: {str(e)}"}}
            )
        ...
        logger.error(f"Unexpected error in upload_server_icon for server {server_id} by user {current_user.user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": {"code": 500, "message": str(e)}}
        )
...
async def delete_server_icon(
    ...
    try:
        ...
        try:
            ...
            if not is_owner and not has_permission:
                logger.warning(f"User {current_user.user_id} denied MANAGE_SERVER for server {sid} icon deletion")
                raise HTTPException(
                    status_code=403,
                    detail={"error": {"code": 403, "message": "Permission denied"}}
                )
            ...
            logger.error(f"Error checking permissions for server {sid} icon deletion: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
                detail={"error": {"code": 500, "message": "Failed to verify permissions"}}
            )
        ...
        try:
            ...
            logger.debug(f"Failed to invalidate cache for server {sid} after icon deletion: {e}")

        return SuccessResponse(success=deleted)
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error in delete_server_icon for server {server_id} by user {current_user.user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={"error": {"code": 500, "message": str(e)}}
        )
