...
class ChunkedUploadSessionRequest(BaseModel):
    ...
    total_size: int = Field(..., gt=0, le=1024 * 1024 * 1024, description="Total file size in bytes (max 1GB)")
...
class ChunkUploadResponse(BaseModel):
    ...
    progress_percent: float = Field(..., description="Overall upload progress percentage")
...
class CompressionStatusResponse(BaseModel):
    ...
    image_compression: bool = Field(..., description="Whether image compression is enabled")
    video_compression: bool = Field(..., description="Whether video compression is enabled")
...
class UploadSessionInfo(BaseModel):
    ...
    status: str = Field(..., description="Session status (pending, uploading, complete, error)")
    expires_at: int = Field(..., description="Session expiration timestamp (Unix)")
