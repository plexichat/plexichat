...
class ICEServerConfig(BaseModel):
    ...
    username: Optional[str] = Field(
        default=None, description="Username for TURN server authentication"
    )
    credential: Optional[str] = Field(
        default=None, description="Credential for TURN server authentication"
    )
...
class ICEServersResponse(BaseModel):
    ...
    ice_servers: List[ICEServerConfig] = Field(
        ..., description="List of ICE server configurations"
    )
...
class VoiceChannelInfoResponse(BaseModel):
    ...
    bitrate: int = Field(
        ..., description="The recommended bitrate for the voice connection"
    )
