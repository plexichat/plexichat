...
def _migrate_messaging_webhook_id(db):
...
def _migrate_media_deduplication_v2(db):
    ...
    try:
        ...
        if db.table_exists("media_file_hashes"):
            if not _column_exists(db, "media_file_hashes", "phash_value"):
                ...
                db.execute("CREATE INDEX IF NOT EXISTS idx_media_file_hashes_phash ON media_file_hashes(phash_value)")
        ...
        if db.table_exists("media_hash_reports"):
            if not _column_exists(db, "media_hash_reports", "phash_value"):
                ...
                db.execute("CREATE INDEX IF NOT EXISTS idx_media_hash_reports_phash ON media_hash_reports(phash_value)")
        ...
        if db.table_exists("media_blocked_hashes"):
            if not _column_exists(db, "media_blocked_hashes", "hash_type"):
                ...
                db.execute("ALTER TABLE media_blocked_hashes ADD COLUMN hash_type TEXT NOT NULL DEFAULT 'sha256'")
                db.execute("CREATE INDEX IF NOT EXISTS idx_media_blocked_hashes_type ON media_blocked_hashes(hash_type)")
