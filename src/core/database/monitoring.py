...
class DatabaseMonitor:
    ...
    def __init__(self, db_config: Dict[str, Any], db_type: str):
        ...
        self._pool_saturation_threshold_percent = alert_thresholds.get(
    ...
    def get_pool_stats(self, engine_stats: Dict[str, Any]) -> Dict[str, Any]:
        with self._lock:
            ...
            if self._connection_acquisitions:
                acq_times = [
                    duration for _, duration in self._connection_acquisitions[-100:]
                ]
            ...
            if self._connection_pool_waits:
                wait_times = [
                    duration for _, duration in self._connection_pool_waits[-100:]
                ]
            ...
            for conn_id, metadata in self._connection_metadata.items():
                if "created_at" in metadata:
                    ...
                    if (
                        self._max_connection_age_seconds > 0
                        and age_seconds > self._max_connection_age_seconds
                    ):
    ...
    def start_pool_monitoring(self, get_stats_cb, reap_cb=None):
        ...
        with self._lock:
            if (
                self._periodic_logging_thread
                and self._periodic_logging_thread.is_alive()
            ):
                return
            ...
            logger.info(
                f"Started pool monitoring thread (interval: {self._periodic_log_interval}s)"
            )
            ...
            try:
                ...
                if reap_cb:
                    try:
                        ...
                        if reaped > 0:
                            logger.info(
                                f"Pool maintenance: reaped {reaped} leaked/idle connections"
                            )
                ...
                context = self._format_log_context(
                    ...
                    utilization_percent=f"{stats.get('utilization_percent', 0):.1f}"
                    if isinstance(stats.get("utilization_percent"), (int, float))
                    else stats.get("utilization_percent"),
                ...
                if (
                    ...
                    and util > self._pool_saturation_threshold_percent
                ):
                    logger.warning(
                ...
                for old in stats.get("old_connections", []):
                    logger.warning(
                        f"Long-lived connection detected - ID: {old['connection_id']}, Age: {old['age_seconds']:.1f}s"
                    )
    ...
    def _cleanup_metrics(self):
        ...
        with self._lock:
            ...
            for error_type in list(self._error_counts.keys()):
                self._error_counts[error_type] = [
                    (ts, et)
                    for ts, et in self._error_counts[error_type]
                    if ts >= window_start
                ]
    ...
    def check_connection_age(self, conn_id: int) -> None:
        ...
        with self._lock:
            ...
            if metadata and "created_at" in metadata:
                ...
                if (
                    self._max_connection_age_seconds > 0
                    and age > self._max_connection_age_seconds
                ):
                    logger.warning(
                        f"Connection {conn_id} has exceeded max age threshold ({age:.1f}s)"
                    )
