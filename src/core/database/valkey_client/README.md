# Valkey Client Sub-package

Refactored from a monolithic `valkey_client.py` into a mixin-based architecture.

## Structure

```
valkey_client/
├── __init__.py    - Package entry point; re-exports ValkeyClient + module-level helpers
├── base.py        - ValkeyClientBase: __init__, connection management, shared state,
...
├── composer.py    - ValkeyClient combining all mixins via multiple inheritance
└── README.md      - This file
```

## Architecture

Each mixin class inherits from `ValkeyClientBase` and provides a focused set of Valkey operations.
The `ValkeyClient` class in `composer.py` combines all mixins via multiple inheritance (MRO).

Cross-mixin calls (e.g., `JSONMixin.set_json` calling `BasicMixin.set`) are declared as stub
methods on `ValkeyClientBase` so type checkers can resolve them. At runtime, Python's MRO
...
```python
from src.core.database.valkey_client import setup, get_client, is_available
