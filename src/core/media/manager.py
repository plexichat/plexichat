...
class MediaManager(BaseManager):
    ...
    def check_file_access(self, filename: str, user_id: int) -> bool:
        ...
        # We look for the file_id in msg_attachments metadata or by matching the filename in the URL
        query = """
        ...
        # 3. Check if it's an avatar or icon (handled by separate routes but we check here for safety)
        # If it's a user's own current avatar
        avatar_row = self._db.fetch_one("SELECT 1 FROM auth_users WHERE avatar_url LIKE '%' || ?", (filename,))
        if avatar_row:
            # Avatars are generally public to anyone who can see the user
            # For strict Zero Trust, we could check mutual servers here too
            return True
            
        # If it's a server icon for a server the user is in
        icon_query = """
