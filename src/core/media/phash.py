...
def compute_phash(image_data: bytes, hash_size: Optional[int] = None) -> Optional[str]:
    ...
    try:
        ...
        phash = imagehash.phash(
            img, hash_size=hash_size, highfreq_factor=cfg["highfreq_factor"]
        )
        return str(phash)
...
def compute_image_hash(
    image_data: bytes, hash_type: Optional[HashType] = None
) -> Optional[ImageHash]:
    """
    ...
    if hash_type is None:
        algo = cfg["algorithm"]
        hash_type = (
            HashType(algo) if algo in [e.value for e in HashType] else HashType.PHASH
        )
