...
from ..exceptions import StorageError, StorageReadError, StorageWriteError
...
class EncryptedStorage(StorageBackendBase):
    ...
    def store(self, file_data: bytes, path: str, content_type: str) -> str:
        ...
        try:
            ...
            enc_path = path + ".enc"
            result = self._backend.store(encrypted_blob, enc_path, "application/octet-stream")
            
            logger.debug(f"Stored encrypted file: {path} ({len(file_data)} -> {len(encrypted_blob)} bytes)")
    ...
    def store_stream(self, stream: BinaryIO, path: str, content_type: str, size: int) -> str:
        ...
        try:
            ...
            result = self._backend.store_stream(output, enc_path, "application/octet-stream", encrypted_size)
            
            logger.debug(f"Stored encrypted stream: {path} ({size} -> {encrypted_size} bytes)")
            return path
    ...
    def retrieve(self, path: str) -> bytes:
        ...
        try:
            if self._backend.exists(enc_path):
                if not self._enabled or not self._encryptor:
                    raise StorageReadError("File is encrypted but encryption is disabled", "encrypted")
