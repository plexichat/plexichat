...
def test_apply_all_pending_success(mock_db, mocker):
    ...
    mocker.patch(
        "src.core.migrations.validator.validate_migration_order", return_value=True
    )
...
def test_validate_integrity_mismatch(mock_db, temp_migrations_dir, mocker):
    ...
    mock_records = [{"version": "001", "status": "completed", "checksum": "wrong_hash"}]
    mocker.patch.object(
        manager.tracker, "get_all_migration_records", return_value=mock_records
    )
    mocker.patch(
        "src.core.migrations.validator.get_migration_files",
        return_value=[("001", str(file1))],
    )
