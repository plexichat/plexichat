...
class TestAuthenticationRequired:
    ...
    def test_send_message_requires_auth(self, test_client):
        ...
        response = test_client.post(
            "/api/v1/channels/123/messages", json={"content": "test"}
        )
        assert response.status_code == 401
...
class TestInvalidTokens:
    ...
    def test_no_bearer_prefix_rejected(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/users/@me", headers={"Authorization": "some_token_123"}
        )
        assert response.status_code == 401
    ...
    def test_jwt_like_token_rejected(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/users/@me", headers={"Authorization": f"Bearer {fake_jwt}"}
        )
        assert response.status_code == 401
    ...
    def test_sql_injection_in_token_rejected(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/users/@me", headers={"Authorization": "Bearer ' OR '1'='1"}
        )
        assert response.status_code == 401
    ...
    def test_extremely_long_token_rejected(self, test_client):
        ...
        long_token = "a" * 10000
        response = test_client.get(
            "/api/v1/users/@me", headers={"Authorization": f"Bearer {long_token}"}
        )
        assert response.status_code == 401
...
class TestTokenInjection:
    ...
    def test_multiple_authorization_headers_rejected(
    ...
    def test_extra_spaces_in_auth_header_rejected(
        self, test_client, create_user_with_token
    ):
