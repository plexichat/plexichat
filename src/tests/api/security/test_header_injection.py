...
class TestHeaderInjection:
    ...
    def test_header_injection_in_content_type(self, test_client, create_user_with_token):
...
class TestAuthorizationHeaderManipulation:
    ...
    def test_multiple_bearer_tokens(self, test_client, create_user_with_token):
        ...
        response = test_client.get(
            "/api/v1/users/@me",
            headers={
                "Authorization": f"Bearer {user['token']} Bearer fake_token"
            }
        )
    ...
    def test_bearer_token_with_semicolon(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/users/@me",
            headers={
                "Authorization": "Bearer fake_token; admin=true"
            }
        )
    ...
    def test_auth_header_with_encoding_tricks(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/users/@me",
            headers={
                "Authorization": "Bearer%20fake_token"
            }
        )
    ...
    def test_case_manipulation_in_bearer(self, test_client, create_user_with_token):
        ...
        for case in test_cases:
            response = test_client.get(
                "/api/v1/users/@me",
                headers={
                    "Authorization": f"{case} {user['token']}"
                }
            )
            assert response.status_code == 401, f"Case variant '{case}' should be rejected"
