...
class TestBodyParameterValidation:
    ...
    def test_negative_ids_rejected(self, test_client, create_user_with_token):
        ...
        response = test_client.get(
            "/api/v1/users/-1", headers={"Authorization": f"Bearer {user['token']}"}
        )
    ...
    def test_zero_id_rejected(self, test_client, create_user_with_token):
        ...
        response = test_client.get(
            "/api/v1/users/0", headers={"Authorization": f"Bearer {user['token']}"}
        )
...
class TestContentLengthValidation:
    ...
    def test_extremely_long_message_rejected(
    ...
    def test_extremely_long_server_name_rejected(
    ...
    def test_empty_required_fields_rejected(self, test_client):
        ...
        response = test_client.post(
            "/api/v1/auth/register", json={"username": "", "email": "", "password": ""}
        )
...
class TestArrayBoundsValidation:
    ...
    def test_too_many_attachments_rejected(
        ...
        response = test_client.post(
            f"/api/v1/channels/{dm.id}/messages",
            headers={"Authorization": f"Bearer {user1['token']}"},
            json={"content": "Test", "attachments": attachments},
        )
    ...
    def test_empty_array_in_batch_operations(self, test_client, create_user_with_token):
        ...
        response = test_client.post(
            "/api/v1/servers",
            headers={"Authorization": f"Bearer {user['token']}"},
            json={"name": "Test Server", "channels": []},
        )
...
class TestTypeValidation:
    ...
    def test_string_where_boolean_expected(
        self, test_client, modules, create_user_with_token
    ):
        ...
        msg = modules.messaging.send_message(
            user_id=user1["user"].id, conversation_id=dm.id, content="Test"
        )
    ...
    def test_array_where_string_expected(self, test_client):
        ...
        response = test_client.post(
            "/api/v1/auth/login",
            json={"username": ["admin", "user"], "password": "TestPass123!"},
        )
...
class TestSpecialCharacterHandling:
    ...
    def test_unicode_in_message_content(
        self, test_client, modules, create_user_with_token
    ):
    ...
    def test_null_bytes_in_content_rejected(
        self, test_client, modules, create_user_with_token
    ):
