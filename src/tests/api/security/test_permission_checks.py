...
class TestResourceOwnership:
    ...
    def test_cannot_delete_other_user_messages(
        ...
        msg = modules.messaging.send_message(
            user_id=user1["user"].id, conversation_id=dm.id, content="Test message"
        )
    ...
    def test_cannot_modify_other_user_profile(
    ...
    def test_cannot_delete_other_user_server(
        self, test_client, create_server_with_owner, create_user_with_token
    ):
    ...
    def test_cannot_kick_members_without_permission(
        self, test_client, modules, create_server_with_owner, create_user_with_token
    ):
...
class TestPrivateDataAccess:
    ...
    def test_cannot_view_other_user_2fa_status(
        self, test_client, create_user_with_token
    ):
...
class TestServerPermissions:
    ...
    def test_non_member_cannot_view_server_channels(
        self, test_client, create_server_with_owner, create_user_with_token
    ):
    ...
    def test_non_member_cannot_view_server_members(
        self, test_client, create_server_with_owner, create_user_with_token
    ):
...
class TestCrossUserOperations:
    ...
    def test_cannot_accept_relationship_for_another_user(
        self, test_client, modules, create_user_with_token
    ):
        ...
        try:
            modules.relationships.send_friend_request(
    ...
    def test_cannot_block_on_behalf_of_another_user(
        self, test_client, create_user_with_token
    ):
        ...
        response = test_client.post(
            ...
            headers={"Authorization": f"Bearer {user1['token']}"},
            json={
                "user_id": str(user3["user"].id),
                "on_behalf_of": str(user2["user"].id),
            },
        )
...
class TestParameterTampering:
    ...
    def test_cannot_set_self_as_admin_via_update(
        self, test_client, create_user_with_token
    ):
    ...
    def test_cannot_change_user_id_in_message(
        self, test_client, modules, create_user_with_token
    ):
    ...
    def test_cannot_transfer_server_ownership_without_permission(
        self, test_client, create_server_with_owner, create_user_with_token
    ):
