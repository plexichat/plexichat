...
class TestApplicationCommands:
    ...
    def test_register_slash_command(self, app_manager):
        ...
        cmd = app_manager.register_command(
            app.id, "greet", "Say hello", command_type=CommandType.CHAT_INPUT
        )
    ...
    def test_register_user_command(self, app_manager):
        ...
        cmd = app_manager.register_command(
            app.id, "user_action", "User action", command_type=CommandType.USER
        )
    ...
    def test_register_message_command(self, app_manager):
        ...
        cmd = app_manager.register_command(
            app.id, "msg_action", "Message action", command_type=CommandType.MESSAGE
        )
    ...
    def test_register_command_with_options(self, app_manager):
        ...
        cmd = app_manager.register_command(app.id, "ban", "Ban a user", options=options)
    ...
    def test_register_guild_command(self, app_manager):
        ...
        cmd = app_manager.register_command(
            app.id, "guild_cmd", "Guild command", server_id=10
        )
...
class TestApplicationOAuth:
    ...
    def test_generate_oauth_url(self, app_manager):
        ...
        app = app_manager.create_application(
            1, "Test App", redirect_uris=["https://example.com/callback"]
        )
...
class TestApplicationRateLimit:
    ...
    def test_rate_limit_enforcement(self, app_manager, monkeypatch):
        """Rate limit enforced."""
        monkeypatch.setitem(
            app_manager._config.get("rate_limits", {}), "requests_per_minute", 2
        )
    ...
    def test_rate_limit_resets(self, app_manager, monkeypatch):
        ...
        monkeypatch.setitem(
            app_manager._config.get("rate_limits", {}), "requests_per_minute", 1
        )
        ...
        app_manager._rate_limits[app.id]["reset_at"] = (
            app_manager._current_time() - 1000
        )
...
class TestApplicationUpdate:
    ...
    def test_update_description(self, app_manager):
        ...
        updated = app_manager.update_application(
            1, app.id, description="New description"
        )
