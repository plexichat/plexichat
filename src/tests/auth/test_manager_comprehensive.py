...
class TestAuthManagerErrorPaths:
    ...
    def test_login_account_lockout(self, auth_manager, test_db, monkeypatch):
        ...
        monkeypatch.setitem(
            auth_manager._get_config("security", {}), "max_failed_attempts", 3
        )
        monkeypatch.setitem(
            auth_manager._get_config("security", {}), "lockout_duration_minutes", 1
        )
    ...
    def test_login_email_not_verified(self, auth_manager, monkeypatch):
        ...
        monkeypatch.setitem(
            auth_manager._get_config("accounts", {}), "require_email_verification", True
        )
    ...
    def test_resend_verification_already_verified(
        self, auth_manager, email_sender, test_db
    ):
        ...
        test_db.execute(
            "UPDATE auth_users SET email_verified = 1 WHERE id = ?", (user.id,)
        )
...
class TestAuthManagerTokenHandling:
    ...
    def test_verify_token_rate_limiting(self, auth_manager, monkeypatch):
        ...
        monkeypatch.setitem(
            auth_manager._get_config("security", {}), "token_verify_rate_limit", 1
        )
        ...
        with pytest.raises(TokenInvalidError) as exc:
            auth_manager.verify_token(result.token, ip)
        assert (
            "rate limit" in str(exc.value).lower() or "many" in str(exc.value).lower()
        )

    def test_verify_token_ip_binding(self, auth_manager, test_db, monkeypatch):
        """Test token IP binding enforcement."""
        monkeypatch.setitem(
            auth_manager._get_config("security", {}), "token_binding", True
        )

        auth_manager.register("testuser", "test@example.com", "Password123!")
        result = auth_manager.login(
            "testuser", "Password123!", ip_address="192.168.1.1"
        )
    ...
    def test_verify_token_user_agent_binding(self, auth_manager, test_db, monkeypatch):
        ...
        with pytest.raises(TokenInvalidError):
            auth_manager.verify_token(
                result.token, ip_address="192.168.1.1", user_agent="Chrome/90.0"
            )
    ...
    def test_token_session_activity_extension(self, auth_manager, test_db, monkeypatch):
        ...
        monkeypatch.setitem(
            auth_manager._get_config("sessions", {}), "extend_on_activity", True
        )
        monkeypatch.setitem(
            auth_manager._get_config("sessions", {}), "extend_threshold_hours", 0
        )
        ...
        original_expires = test_db.fetch_one(
            "SELECT expires_at FROM auth_sessions WHERE id = ?", (result.session.id,)
        )["expires_at"]
...
class TestAuthManagerTwoFactor:
    ...
    def test_regenerate_backup_codes_wrong_password(self, auth_manager, test_db):
        ...
        test_db.execute(
            "UPDATE auth_users SET totp_enabled = 1 WHERE id = ?", (user.id,)
        )
...
class TestAuthManagerBots:
    ...
    def test_create_bot_no_permission(self, auth_manager, test_db):
        ...
        test_db.execute(
            "UPDATE auth_users SET permissions = '{}' WHERE id = ?", (user.id,)
        )
    ...
    def test_create_bot_limit_exceeded(self, auth_manager, monkeypatch):
        ...
        monkeypatch.setitem(
            auth_manager._get_config("accounts", {}), "max_bots_per_user", 1
        )
    ...
    def test_reset_password_weak_password(self, auth_manager, test_db, email_sender):
        ...
        test_db.execute(
            ...
            (
                ...
                token_hash,
                "reset_password",
                auth_manager._current_time(),
                auth_manager._current_time() + 3600,
            ),
        )
