...
class TestSessions:
    ...
    def test_logout_all_devices(self, db_and_auth):
        ...
        user = auth.register("logoutall", "logoutall@example.com", "TestPass123!")
        
        result1 = auth.login("logoutall", "TestPass123!")
        result2 = auth.login("logoutall", "TestPass123!")
        result3 = auth.login("logoutall", "TestPass123!")
    ...
    def test_logout_all_except_current(self, db_and_auth):
        ...
        user = auth.register("logoutexcept", "logoutexcept@example.com", "TestPass123!")
        
        result1 = auth.login("logoutexcept", "TestPass123!")
        result2 = auth.login("logoutexcept", "TestPass123!")
        result3 = auth.login("logoutexcept", "TestPass123!")
    ...
    def test_revoke_session_wrong_user(self, db_and_auth):
        ...
        user1 = auth.register("revokeuser1", "revokeuser1@example.com", "TestPass123!")
        user2 = auth.register("revokeuser2", "revokeuser2@example.com", "TestPass123!")
        
        result = auth.login("revokeuser1", "TestPass123!")
        session_id = int(result.token.split(".")[0])
    ...
    def test_session_limit_revokes_oldest(self, db_and_auth):
        ...
        user = auth.register("sessionlimit", "sessionlimit@example.com", "TestPass123!")
        
        # Create many sessions (config has max_per_user = 10)
        for i in range(12):
            auth.login("sessionlimit", "TestPass123!")
            time.sleep(0.01)
