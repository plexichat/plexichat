...
def db_and_modules(test_env, request):
    ...
    yield db, auth, messaging, servers, embeds
...
def base_users(db_and_modules):
    ...
    return user1, user2, user3, auth, messaging, servers, embeds


@pytest.fixture
def users_with_dm(base_users):
    """Create users with a DM conversation and message."""
    user1, user2, user3, auth, messaging, servers, embeds = base_users

    dm = messaging.create_dm(user1.id, user2.id)
    msg = messaging.send_message(user1.id, dm.id, "Test message for embeds")
...
def fresh_users_with_dm(db_and_modules):
    ...
    return user1, user2, dm, msg, embeds, messaging
...
def users_with_server(db_and_modules):
    ...
    msg = messaging.send_message(owner.id, group.id, "Server message for embeds")

    return owner, member, server, group, msg, servers, embeds, messaging
...
def group_with_message(db_and_modules):
    ...
    group = messaging.create_group(owner.id, f"Test Group {unique_id}", [member1.id])
    msg = messaging.send_message(owner.id, group.id, "Group message for embeds")

    return owner, member1, group, msg, messaging, embeds
