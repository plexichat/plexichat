...
class TestTitleValidation:
    ...
    def test_title_max_length(self, db, auth_manager, embeds_manager):
        ...
        title = "a" * 256
        embed = embeds_manager.create_embed(user_id=user.id, title=title)

        assert len(embed.title) == 256

    def test_title_exceeds_max_length(self, db, auth_manager, embeds_manager):
        ...
        with pytest.raises(EmbedValidationError) as exc_info:
            embeds_manager.create_embed(user_id=user.id, title=title)
...
class TestDescriptionValidation:
    ...
    def test_description_max_length(self, db, auth_manager, embeds_manager):
        ...
        desc = "a" * 4096
        embed = embeds_manager.create_embed(user_id=user.id, description=desc)

        assert len(embed.description) == 4096

    def test_description_exceeds_max_length(self, db, auth_manager, embeds_manager):
        ...
        with pytest.raises(EmbedValidationError) as exc_info:
            embeds_manager.create_embed(user_id=user.id, description=desc)
...
class TestUrlValidation:
    ...
    def test_valid_https_url(self, db, auth_manager, embeds_manager):
    ...
    def test_invalid_url_format(self, db, auth_manager, embeds_manager):
...
class TestColorValidation:
    ...
    def test_valid_hex_color_with_hash(self, db, auth_manager, embeds_manager):
    ...
    def test_valid_hex_color_without_hash(self, db, auth_manager, embeds_manager):
        ...
        embed = embeds_manager.create_embed(
            user_id=user.id, title="Color Test", color="00FF00"
        )

        assert embed.color == "#00FF00"

    def test_valid_short_hex_color(self, db, auth_manager, embeds_manager):
        ...
        embed = embeds_manager.create_embed(
            user_id=user.id, title="Short Color", color="#F00"
        )

        assert embed.color == "#FF0000"

    def test_invalid_color_format(self, db, auth_manager, embeds_manager):
    ...
    def test_invalid_hex_color(self, db, auth_manager, embeds_manager):
        ...
        with pytest.raises(EmbedValidationError):
            embeds_manager.create_embed(
                user_id=user.id, title="Invalid Hex", color="#GGGGGG"
            )
...
class TestTimestampValidation:
    ...
    def test_valid_iso8601_timestamp(self, db, auth_manager, embeds_manager):
    ...
    def test_valid_date_only_timestamp(self, db, auth_manager, embeds_manager):
    ...
    def test_invalid_timestamp_format(self, db, auth_manager, embeds_manager):
...
class TestTotalCharacterLimit:
    ...
    def test_total_chars_at_limit(self, db, auth_manager, embeds_manager):
    ...
    def test_total_chars_exceeds_limit(self, db, auth_manager, embeds_manager):
...
class TestValidateEmbedFunction:
    ...
    def test_validate_valid_embed(self, db, auth_manager, embeds_manager):
        """Test validating valid embed data."""

        result = embeds_manager.validate_embed(
    ...
    def test_validate_invalid_embed(self, db, auth_manager, embeds_manager):
    ...
    def test_validate_returns_total_chars(self, db, auth_manager, embeds_manager):
