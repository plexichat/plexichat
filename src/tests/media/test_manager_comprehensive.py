...
class TestMediaErrors:
    ...
    def test_invalid_file_type(self, media_manager, monkeypatch):
        """Invalid file type."""
        monkeypatch.setitem(
            media_manager._config["allowed_types"], "image", ["image/png"]
        )
    ...
    def test_rate_limit_minute(self, media_manager, test_db, monkeypatch):
        ...
        monkeypatch.setitem(
            media_manager._config["rate_limit"], "uploads_per_minute", 1
        )
    ...
    def test_rate_limit_daily_size(self, media_manager, test_db, monkeypatch):
        ...
        if "rate_limit" not in media_manager._config:
            media_manager._config["rate_limit"] = {"enabled": True}
        monkeypatch.setitem(
            media_manager._config["rate_limit"], "max_total_size_per_day", 1000
        )
    ...
    def test_upload_image(self, media_manager):
        ...
        result = media_manager.upload_file(
            1, _create_minimal_jpeg(), "test.jpg", "image/jpeg"
        )
    ...
    def test_upload_file(self, media_manager):
        """Upload general file."""
        result = media_manager.upload_file(
            1, b"fake_file_data", "test.txt", "text/plain"
        )
        assert result is not None
...
class TestMediaValidation:
    ...
    def test_detect_media_type_other(self, media_manager):
        ...
        assert (
            media_manager._detect_media_type("application/octet-stream")
            == MediaType.OTHER
        )
...
class TestMediaAttachments:
    ...
    def test_upload_attachment(self, media_manager):
        ...
        attachment = media_manager.upload_attachment(
            1, b"content", "file.txt", "text/plain"
        )
...
class TestMediaRateLimit:
    ...
    def test_rate_limit_window_counts(self, media_manager):
        ...
        minute_count = media_manager._get_rate_limit_count(
            1,
            "minute",
            media_manager._get_timestamp() // 1000
            - (media_manager._get_timestamp() // 1000 % 60),
        )
    ...
    def test_rate_limit_size_tracking(self, media_manager):
        ...
        day_window = media_manager._get_timestamp() // 1000 - (
            media_manager._get_timestamp() // 1000 % 86400
        )
...
class TestMediaProxy:
    ...
    def test_proxy_url_force_refresh(self, media_manager, monkeypatch):
        ...
        if hasattr(media_manager, "_proxy") and media_manager._proxy:
            monkeypatch.setattr(media_manager._proxy, "fetch", mock_fetch)
            proxied = media_manager.proxy_url(
                "https://example.com/image.jpg", force_refresh=True
            )
            assert proxied is not None
        else:
            try:
    ...
    def test_get_proxied_content(self, media_manager, monkeypatch):
        ...
        if hasattr(media_manager, "_proxy") and media_manager._proxy:
            ...
            content, content_type = media_manager.get_proxied_content(
...
class TestMediaAutoRouting:
    ...
    def test_should_route_to_database(self, media_manager, monkeypatch):
        ...
        monkeypatch.setitem(
            ...
            {"enabled": True, "max_size": 1024, "content_types": ["text/plain"]},
        )
