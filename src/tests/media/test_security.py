...
class TestFileUploadValidation:
    ...
    def test_filename_sanitization_removes_path_components(
    ...
    def test_filename_sanitization_removes_control_characters(
        self, media_module, user_pool, sample_image_bytes
    ):
    ...
    def test_filename_sanitization_limits_length(
        self, media_module, user_pool, sample_image_bytes
    ):
...
class TestMIMETypeVerification:
    ...
    def test_jpeg_magic_byte_validation(
        self, media_module, user_pool, sample_image_bytes
    ):
    ...
    def test_mime_type_spoofing_text_as_image(self, media_module, user_pool):
        ...
        assert (
            "declared type" in error_msg
            or "signature" in error_msg
            or "match" in error_msg
        )
    ...
    def test_mime_type_spoofing_html_as_image(self, media_module, user_pool):
        ...
        html_data = (
            b"<!DOCTYPE html><html><body><script>alert('XSS')</script></body></html>"
        )
    ...
    def test_wrong_image_type_png_as_jpeg(
        self, media_module, user_pool, sample_png_bytes
    ):
...
class TestImageProcessingExploits:
    ...
    def test_decompression_bomb_protection(self, media_module, user_pool):
        ...
        try:
            ...
            assert any(
                word in error_msg
                for word in ["pixel", "dimension", "size", "limit", "metadata"]
            )
        except MemoryError:
...
class TestPathTraversalAttempts:
    ...
    def test_windows_path_in_filename(
        self, media_module, user_pool, sample_image_bytes
    ):
...
class TestCDNProxySecurity:
    ...
    def test_proxy_blocks_internal_hostnames(self, modules, temp_upload_dir):
        ...
        for url in internal_hostnames:
            ...
            assert (
                "internal" in str(exc_info.value).lower()
                or "not allowed" in str(exc_info.value).lower()
            )
    ...
    def test_proxy_blocks_non_http_schemes(self, modules, temp_upload_dir):
        ...
        for url in dangerous_schemes:
            ...
            assert (
                "scheme" in str(exc_info.value).lower()
                or "not allowed" in str(exc_info.value).lower()
            )

        config._config_instance.config["media"] = original_media_config

    def test_proxy_enforces_content_type_whitelist(
        self, modules, temp_upload_dir, mocker
    ):
...
class TestMaliciousFileContent:
    ...
    def test_svg_with_javascript_allowed_as_svg(self, media_module, user_pool):
        ...
        try:
            ...
            assert (
                "type not allowed" in str(e).lower() or "content type" in str(e).lower()
            )
...
class TestRateLimitSecurity:
    ...
    def test_per_minute_upload_limit(
        self, modules, temp_upload_dir, user_pool, sample_image_bytes, monkeypatch
    ):
        ...
        monkeypatch.setitem(
            media._get_manager()._config, "rate_limit", test_config["rate_limit"]
        )
    ...
    def test_per_hour_upload_limit(
        self, modules, temp_upload_dir, user_pool, sample_image_bytes, monkeypatch
    ):
    ...
    def test_daily_size_limit(
        self, modules, temp_upload_dir, user_pool, sample_image_bytes
    ):
