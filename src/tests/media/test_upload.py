...
class TestMediaAsync:
    ...
    async def test_concurrent_uploads(self, media_module, user_pool, sample_image_bytes):
    ...
    async def test_thumbnail_regeneration_on_demand(self, media_module, user_pool, sample_image_bytes):
    ...
    async def test_large_file_simulated_chunked_upload(self, media_module, user_pool):
        ...
        retrieved_data, _ = await asyncio.to_thread(media_module.get_file_data, result.file_id)
