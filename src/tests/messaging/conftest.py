...
def base_users(db_and_modules):
    """Create base test users once per module - these are reused."""
    ...
    return user1, user2, user3, auth, messaging


@pytest.fixture
def users(base_users):
    """Get test users - reuses module-scoped base_users."""
    user1, user2, user3, auth, messaging = base_users
    return user1, user2, user3, messaging


@pytest.fixture
def dm_conversation(base_users):
    """Create a DM conversation between user1 and user2."""
    user1, user2, user3, auth, messaging = base_users
    
    # DMs are reused if they exist, so this is efficient
