...
class TestMessagesAsync:
    ...
    async def test_send_message_success(self, dm_conversation):
        ...
        msg = await asyncio.to_thread(
            messaging.send_message, user1.id, dm.id, "Hello, world!"
        )
    ...
    async def test_concurrent_message_sending(self, dm_conversation):
        ...
        tasks = [
            asyncio.to_thread(messaging.send_message, user1.id, dm.id, f"Msg {i}")
            for i in range(20)
        ]
    ...
    async def test_complex_pagination(self, dm_conversation):
        ...
        for i in range(10):
            m = await asyncio.to_thread(
                messaging.send_message, user1.id, dm.id, f"Msg {i}"
            )
            msgs.append(m)
        ...
        page = await asyncio.to_thread(
            messaging.get_messages, user1.id, dm.id, limit=4, before_id=pivot.id
        )
    ...
    async def test_bulk_delete_performance(self, dm_conversation):
        ...
        for i in range(50):
            m = await asyncio.to_thread(
                messaging.send_message, user1.id, dm.id, f"Delete test {i}"
            )
            msgs.append(m)

        # Delete half of them in parallel
        tasks = [
            asyncio.to_thread(messaging.delete_message, user1.id, m.id)
            for m in msgs[:25]
        ]
    ...
    async def test_reply_chain_integrity(self, dm_conversation):
        ...
        m2 = await asyncio.to_thread(
            messaging.send_message, user2.id, dm.id, "Reply 1", reply_to_id=m1.id
        )
        m3 = await asyncio.to_thread(
            messaging.send_message, user1.id, dm.id, "Reply 2", reply_to_id=m2.id
        )
