...
class TestNotificationErrors:
    ...
    def test_validate_mention_nonmentionable_role(self, notification_manager, test_db):
        ...
        test_db.execute("CREATE TABLE IF NOT EXISTS srv_servers (id INTEGER PRIMARY KEY, name TEXT, owner_id INTEGER, created_at INTEGER, updated_at INTEGER)")
        test_db.execute("CREATE TABLE IF NOT EXISTS srv_roles (id INTEGER PRIMARY KEY, server_id INTEGER, name TEXT, permissions TEXT, position INTEGER, mentionable INTEGER, created_at INTEGER, updated_at INTEGER, color INTEGER, hoist INTEGER)")
