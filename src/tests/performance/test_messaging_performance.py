...
class TestMessagingPerformance:
    ...
    def test_send_message_performance(self, benchmark, messaging_manager, test_dm):
    ...
    def test_get_messages_performance(self, benchmark, messaging_manager, test_dm):
        ...
        def get_messages():
            return messaging_manager.get_messages(
    ...
    def test_bulk_send_performance(self, benchmark, messaging_manager, test_dm):
        ...
        def bulk_send():
            ...
            for i in range(100):
                msg = messaging_manager.send_message(
                    user_id=user1.id, conversation_id=dm.id, content=f"Bulk message {i}"
                )
                messages.append(msg)
            return messages
    ...
    def test_concurrent_send_performance(self, benchmark, messaging_manager, test_dm):
    ...
    def test_message_search_performance(self, benchmark, messaging_manager, test_dm):
        """Test message search performance."""
        dm, user1, user2 = test_dm

        for i in range(200):
            content = (
                f"Searchable message {i} with keyword"
                if i % 10 == 0
                else f"Regular message {i}"
            )
            messaging_manager.send_message(
                user_id=user1.id, conversation_id=dm.id, content=content
            )

        def search_messages():
            all_messages = messaging_manager.get_messages(
                user_id=user1.id, conversation_id=dm.id, limit=500
            )
    ...
    def test_edit_message_performance(self, benchmark, messaging_manager, test_dm):
        """Test message editing performance."""
        dm, user1, user2 = test_dm

        msg = messaging_manager.send_message(
            user_id=user1.id, conversation_id=dm.id, content="Original content"
        )

        def edit_message():
            return messaging_manager.edit_message(
                user_id=user1.id, message_id=msg.id, content="Edited content"
            )

        result = benchmark(edit_message)
        assert result.content == "Edited content"

    def test_conversation_list_performance(
        self, benchmark, messaging_manager, user_pool
    ):
        ...
        def get_conversations():
            return messaging_manager.get_conversations(user.id)
...
class TestMessagingMemory:
    ...
    def test_send_message_memory_leak(self, messaging_manager, test_dm, memory_tracker):
    ...
    def test_get_messages_memory_leak(self, messaging_manager, test_dm, memory_tracker):
        """Check for memory leaks during repeated message retrievals."""
        dm, user1, user2 = test_dm

        for i in range(100):
            messaging_manager.send_message(
                user_id=user1.id, conversation_id=dm.id, content=f"Test message {i}"
            )
    ...
    def test_large_message_memory(self, messaging_manager, test_dm, memory_tracker):
...
class TestMessagingDegradation:
    ...
    def test_send_performance_degradation(self, messaging_manager, test_dm):
        ...
        for i in range(200):
            start = datetime.now()
            messaging_manager.send_message(
                user_id=user1.id, conversation_id=dm.id, content=f"Degradation test {i}"
            )
    ...
    def test_large_conversation_performance(self, messaging_manager, test_dm):
    ...
    def test_concurrent_send_scaling(self, messaging_manager, test_dm):
