...
class TestConnectionLimits:
    ...
    def test_many_concurrent_conversations(self, modules, stress_test_users):
        ...
        assert avg_time < 0.5, f"Get conversations too slow with 29 conversations: {avg_time}s"
    ...
    def test_concurrent_server_operations_stress(self, modules, load_test_server):
        ...
        def mixed_operation(op_id):
            ...
            if op_id % 4 == 0:
                ...
                return modules.messaging.send_message(user.id, conv_id, f"Stress {op_id}")
            elif op_id % 4 == 2:
...
class TestRecovery:
    ...
    def test_memory_recovery_after_load(self, modules, test_dm, memory_tracker):
        ...
        recovery_ratio = (peak_increase - final_increase) / peak_increase if peak_increase > 0 else 0
        
        assert recovery_ratio > 0.3, f"Poor memory recovery: {recovery_ratio*100:.1f}%"
