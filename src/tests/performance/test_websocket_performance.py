...
class TestWebSocketPerformance:
    ...
    @pytest.mark.asyncio
    async def test_event_dispatch_performance(self, benchmark, websocket_manager, test_user_with_token):
    ...
    async def test_broadcast_performance(self, websocket_manager, stress_test_users):
        ...
        assert count == 50
        assert elapsed < 0.5, f"Broadcasting to 50 connections took {elapsed}s (expected < 0.5s)"
    ...
    async def test_heartbeat_performance(self, websocket_manager, test_user_with_token):
        ...
        assert avg_time < 0.001, f"Average heartbeat time {avg_time}s (expected < 0.001s)"
...
class TestWebSocketMemory:
    ...
    @pytest.mark.asyncio
    async def test_connection_memory_leak(self, websocket_manager, stress_test_users, memory_tracker):
    ...
    @pytest.mark.asyncio
    async def test_event_dispatch_memory_leak(self, websocket_manager, test_user_with_token, memory_tracker):
...
class TestWebSocketDegradation:
    ...
    @pytest.mark.asyncio
    async def test_sustained_event_throughput(self, websocket_manager, test_user_with_token):
        ...
        for batch in range(10):
            ...
            for i in range(100):
                event = Event(
                    event_type=EventType.MESSAGE_CREATE,
                    data={"content": f"Sustained test {batch}_{i}", "channel_id": "123"}
                )
                await dispatcher.dispatch_event(event, [user.id])
