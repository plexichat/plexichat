...
def measure_throughput(operation: Callable, duration_seconds: float = 1.0) -> Dict[str, Any]:
    """
...
class LoadPattern:
    ...
    @staticmethod
    def ramp_up(start_rate: int, end_rate: int, duration: float, steps: int = 10) -> List[float]:
    ...
    @staticmethod
    def burst(burst_rate: int, burst_duration: float, idle_duration: float, num_bursts: int) -> List[float]:
        """Burst load pattern."""
        times = []
...
def compare_performance(baseline: PerformanceStats, current: PerformanceStats) -> Dict[str, Any]:
    """
