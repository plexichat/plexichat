...
class PerformanceTracker:
    ...
    def generate_report(self) -> str:
        ...
        lines = [
            ...
            f"Average time: {sum(self.test_times.values()) / len(self.test_times):.3f}s"
            if self.test_times
            else "N/A",
            "",
        ]
        ...
        if very_slow_tests:
            lines.append(
                f"⚠ Very slow tests (>{self.very_slow_threshold}s): {len(very_slow_tests)}"
            )
...
class CoverageEnforcer:
    ...
    def generate_report(self, results: Dict[str, tuple]) -> str:
        ...
        for path, (actual, threshold, passed) in results.items():
            ...
            lines.append(
                f"{status} {path:40s} | Target: {threshold:5.1f}% | Actual: {actual:5.1f}%"
            )
...
class TestMetricsCollector:
    ...
    def __init__(self):
        ...
        self.tests_by_module = defaultdict(
            lambda: {"passed": 0, "failed": 0, "skipped": 0}
        )
        self.tests_by_marker = defaultdict(
            lambda: {"passed": 0, "failed": 0, "skipped": 0}
        )
    ...
    def generate_report(self) -> str:
        ...
        lines = [
            ...
            f"  Passed:  {self.passed:5d} ({self.passed / self.total_tests * 100:5.1f}%)"
            if self.total_tests > 0
            else "  Passed:  0",
            f"  Failed:  {self.failed:5d} ({self.failed / self.total_tests * 100:5.1f}%)"
            if self.total_tests > 0
            else "  Failed:  0",
            f"  Skipped: {self.skipped:5d} ({self.skipped / self.total_tests * 100:5.1f}%)"
            if self.total_tests > 0
            else "  Skipped: 0",
            "",
        ]
        ...
        if self.tests_by_marker:
            ...
            for marker, counts in sorted(self.tests_by_marker.items()):
                ...
                lines.append(
                    f"  {marker:20s}: {total:4d} tests ({counts['passed']} passed, {counts['failed']} failed)"
                )
            lines.append("")
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if security_tracker.has_violations():
        terminalreporter.write_line("")
        terminalreporter.write_line(
            "✗ CRITICAL: Security violations detected. Build must fail.",
            red=True,
            bold=True,
        )
