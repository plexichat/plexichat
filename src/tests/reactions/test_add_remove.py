...
class TestAddReaction:
    ...
    def test_add_reaction_success(self, users_with_dm):
    ...
    def test_add_reaction_other_user(self, users_with_dm):
        """Test another user can add same reaction."""
        user1, user2, dm, msg, reactions = users_with_dm

        reactions.add_reaction(user1.id, msg.id, "heart")
        reaction2 = reactions.add_reaction(user2.id, msg.id, "heart")

        assert reaction2 is not None
        assert reaction2.user_id == user2.id

    def test_add_multiple_different_reactions(self, users_with_dm):
        """Test user can add multiple different reactions."""
        user1, user2, dm, msg, reactions = users_with_dm

        r1 = reactions.add_reaction(user1.id, msg.id, "smile")
        r2 = reactions.add_reaction(user1.id, msg.id, "laugh")

        assert r1.emoji == "smile"
        assert r2.emoji == "laugh"

    def test_add_duplicate_reaction_fails(self, fresh_users_with_dm):
        """Test adding same reaction twice fails."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        reactions.add_reaction(user1.id, msg.id, "star")

        with pytest.raises(ReactionExistsError):
            reactions.add_reaction(user1.id, msg.id, "star")

    def test_add_reaction_nonexistent_message(self, users_with_dm):
        """Test adding reaction to nonexistent message fails."""
        user1, user2, dm, msg, reactions = users_with_dm

        with pytest.raises(MessageNotFoundError):
            reactions.add_reaction(user1.id, 999999999, "thumbsup")

    def test_add_reaction_not_participant(self, db_and_modules):
        ...
        msg = messaging.send_message(user1.id, dm.id, "Private message")

        with pytest.raises(MessageNotFoundError):
            reactions.add_reaction(outsider.id, msg.id, "thumbsup")


class TestRemoveReaction:
    """Tests for removing reactions."""

    def test_remove_reaction_success(self, fresh_users_with_dm):
        """Test removing a reaction successfully."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        reactions.add_reaction(user1.id, msg.id, "wave")
        result = reactions.remove_reaction(user1.id, msg.id, "wave")
    ...
    def test_remove_nonexistent_reaction_fails(self, fresh_users_with_dm):
        """Test removing reaction that does not exist fails."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        with pytest.raises(ReactionNotFoundError):
            reactions.remove_reaction(user1.id, msg.id, "nonexistent")

    def test_remove_other_users_reaction_fails(self, fresh_users_with_dm):
        """Test cannot remove another user's reaction."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        reactions.add_reaction(user1.id, msg.id, "fire")

        with pytest.raises(ReactionNotFoundError):
            reactions.remove_reaction(user2.id, msg.id, "fire")
    ...
    def test_remove_reaction_nonexistent_message(self, users_with_dm):
        ...
        with pytest.raises(MessageNotFoundError):
            reactions.remove_reaction(user1.id, 999999999, "thumbsup")


class TestToggleReaction:
    """Tests for toggling reactions (add if not exists, remove if exists)."""

    def test_add_remove_add_cycle(self, fresh_users_with_dm):
        """Test adding, removing, and re-adding a reaction."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        r1 = reactions.add_reaction(user1.id, msg.id, "cycle")
        assert r1 is not None

        reactions.remove_reaction(user1.id, msg.id, "cycle")

        r2 = reactions.add_reaction(user1.id, msg.id, "cycle")
        assert r2 is not None
        assert r2.id != r1.id

    def test_multiple_users_add_remove(self, fresh_users_with_dm):
        """Test multiple users adding and removing same reaction."""
        user1, user2, dm, msg, reactions, relationships = fresh_users_with_dm

        reactions.add_reaction(user1.id, msg.id, "multi")
