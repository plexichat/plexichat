...
class TestSessionHijacking:
    ...
    def test_session_user_agent_binding(self, modules, user_pool):
        ...
        try:
            token_info = modules.auth.verify_token(result.token, user_agent=user_agent)
            assert token_info.valid
        except Exception:
            pass
    ...
    def test_session_device_tracking(self, modules, user_pool):
        ...
        modules.auth.login(
            username=username, password=password, device_info=device_info
        )
    ...
    def test_suspicious_activity_detection(self, modules, user_pool):
        ...
        try:
            modules.auth.verify_token(result.token, ip_address="10.0.0.1")
        except Exception:
            pass
    ...
    def test_password_change_revokes_sessions(self, modules, user_pool):
        ...
        try:
            modules.auth.change_password(
                user_id=user.id, old_password=password, new_password="NewPass123!"
            )
