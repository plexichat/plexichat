...
class TestServersAsync:
    ...
    async def test_permission_propagation(self, user_pool, modules):
        ...
        assert not await asyncio.to_thread(
            servers.has_permission, member.id, server.id, "server.manage"
        )

        # Assign role
        await asyncio.to_thread(
            servers.assign_role, owner.id, server.id, member.id, mod_role.id
        )
    ...
    async def test_ownership_transfer_integrity(self, user_pool, modules):
        ...
        server = await asyncio.to_thread(
            servers.create_server, owner.id, "Transfer Server"
        )
        await asyncio.to_thread(servers.add_member, server.id, new_owner.id)

        # Transfer
        await asyncio.to_thread(
            servers.transfer_ownership, owner.id, server.id, new_owner.id
        )

        # Verify
        updated_server = await asyncio.to_thread(
            servers.get_server, server.id, new_owner.id
        )
        ...
        assert not await asyncio.to_thread(
            servers.has_permission, owner.id, server.id, "administrator"
    ...
    async def test_bulk_member_management(self, user_pool, modules):
        ...
        server = await asyncio.to_thread(
            servers.create_server, owner.id, "Raid Test Server"
        )
        ...
        fetched_server = await asyncio.to_thread(
            servers.get_server, server.id, owner.id
        )
