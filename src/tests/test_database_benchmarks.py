...
def calculate_benchmark_stats(execution_times: List[float], 
...
def test_single_query_execution_sqlite(benchmark_db_sqlite, benchmark_data):
    ...
    db.execute(
        ...
        (benchmark_data["names"][0], benchmark_data["emails"][0], benchmark_data["data"]),
        auto_commit=True
    )
    ...
    logger.info(f"Single query execution benchmark: {result.mean_ms:.3f}ms (target: 10ms)")
    assert result.exceeded_target_percent == 0, f"Query execution exceeded 10ms target in {result.exceeded_target_count} cases"
...
def test_bulk_insert_sqlite(benchmark_db_sqlite, benchmark_data):
    ...
    logger.info(f"Bulk insert benchmark: {result.mean_ms:.3f}ms per insert (target: 5ms)")
...
def test_query_with_join_sqlite(benchmark_db_sqlite, benchmark_data):
    ...
    for i in range(50):
        db.execute(
            "INSERT INTO benchmark_data (name, email, data) VALUES (?, ?, ?)",
            (benchmark_data["names"][i], f"join_test_{i}@example.com", benchmark_data["data"]),
            auto_commit=True
        )
    ...
    result.database_type = "SQLite"
    result.operation = "SELECT ... FROM benchmark_data bd LEFT JOIN benchmark_profiles bp"
...
def test_connection_acquisition_sqlite(benchmark_db_sqlite, benchmark_data):
    ...
    if hasattr(benchmark_db_sqlite._local, "connection") and benchmark_db_sqlite._local.connection:
    ...
    logger.info(f"Connection acquisition benchmark: {result.mean_ms:.3f}ms (target: 50ms)")
    assert result.exceeded_target_percent == 0, f"Connection acquisition exceeded 50ms in {result.exceeded_target_count} cases"
...
def test_concurrent_connection_acquisition(benchmark_db_sqlite):
    ...
    logger.info(f"Concurrent connection acquisition: {result.mean_ms:.3f}ms (target: 50ms)")
...
def test_transaction_commit_sqlite(benchmark_db_sqlite, benchmark_data):
    ...
    for i in range(iterations):
        with BenchmarkTimer() as timer:
            ...
            db.execute(
                "INSERT INTO benchmark_data (name, email, data) VALUES (?, ?, ?)",
                (f"transaction_test_{i}", f"tx_{i}@example.com", benchmark_data["data"]),
                auto_commit=False
            )
            db.commit()
        execution_times.append(timer.elapsed_ms)
...
def test_placeholder_conversion_performance(benchmark_db_sqlite):
    ...
    test_queries = [
        ...
        ("Multiple placeholders", "SELECT * FROM table WHERE id = ? AND name = ? AND email = ?"),
        ("With quotes", "SELECT * FROM table WHERE id = ? AND name LIKE ? AND data = 'don''t use ?'"),
        ("Complex join", "SELECT t1.* FROM table1 t1 JOIN table2 t2 ON t1.id = ? AND t2.id = ? WHERE t1.name = ?"),
    ]
    ...
    for query_name, result in execution_times_by_query.items():
        assert result.mean_ms < 1.0, f"Placeholder conversion too slow for {query_name}: {result.mean_ms:.4f}ms"
...
@pytest.mark.benchmark
@pytest.mark.parametrize("operation_name,query,params", [
...
def test_database_operation_performance(benchmark_db_sqlite, operation_name, query, params):
...
def performance_baseline():
    ...
    return {
        ...
        "connection_acquisition_ms": {"target": 50.0, "warning": 40.0, "critical": 45.0},
...
@pytest.mark.benchmark
def test_generate_performance_report(benchmark_db_sqlite, benchmark_data, performance_baseline, tmp_path):
    ...
    for i in range(50):
        with BenchmarkTimer() as timer:
            db.execute(
                "SELECT * FROM benchmark_data LIMIT 1",
                auto_commit=False
            )
        execution_times.append(timer.elapsed_ms)
    ...
    logger.info(f"Performance report verified: {loaded_report['summary']['total_benchmarks']} benchmarks")


@pytest.mark.benchmark
def test_performance_regression_detection(benchmark_db_sqlite, benchmark_data, performance_baseline):
    ...
    logger.info(f"Regression detection: baseline={baseline_mean:.3f}ms, current={current_mean:.3f}ms, threshold={threshold:.3f}ms")
    
    if regression_detected:
        logger.warning(f"Performance regression detected: {current_mean:.3f}ms > {threshold:.3f}ms")
    
    # For testing, we allow regression but log it
    assert current_mean < threshold * 1.5, f"Critical regression: {current_mean:.3f}ms exceeds {threshold * 1.5:.3f}ms"
...
def test_high_volume_inserts(benchmark_db_sqlite, benchmark_data):
    ...
    logger.info(f"High-volume insert: {timer.elapsed_ms:.1f}ms total, {time_per_insert:.3f}ms per insert")
    assert time_per_insert < 5.0, f"Insert too slow: {time_per_insert:.3f}ms per operation"
...
def test_concurrent_query_stress(benchmark_db_sqlite, benchmark_data):
    ...
    def stress_query():
        ...
        for i in range(queries_per_thread):
            start = time.time()
            db.execute("SELECT * FROM benchmark_data WHERE id = ?", (i % 100 + 1,), auto_commit=False)
    ...
    if execution_times:
        result = calculate_benchmark_stats(execution_times)
        logger.info(f"Stress test: {result.mean_ms:.3f}ms avg query time ({len(execution_times)} total queries)")
        assert result.mean_ms < 20.0, f"Stress test failed: avg query time {result.mean_ms:.3f}ms exceeds limit"
