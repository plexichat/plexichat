...
def db_and_modules(test_env, request):
    ...
    # Re-initialize voice
    voice._manager = None
    voice._setup_complete = False
    voice.setup(db, auth, servers, relationships, presence)

    yield db, auth, servers, relationships, presence, voice
...
def base_users(db_and_modules):
    ...
    return user1, user2, user3, user4, auth, servers, relationships, presence, voice


@pytest.fixture
def users(base_users):
    """Get test users."""
    user1, user2, user3, user4, auth, servers, relationships, presence, voice = base_users
    return user1, user2, user3, user4, voice
...
def fresh_users(db_and_modules):
    ...
    return user1, user2, voice


@pytest.fixture
def server_with_voice(db_and_modules):
    ...
    server = servers.create_server(owner.id, f"Voice Test Server {unique_id}")
    ...
    stage_channel = servers.create_channel(
        owner.id, server.id, "stage-talk",
        channel_type=servers.ChannelType.STAGE
    )

    return owner, member1, member2, server, voice_channel, stage_channel, servers, voice
...
def server_with_moderator(db_and_modules):
    ...
    return owner, moderator, member, server, voice_channel, stage_channel, servers, voice
