...
If `allow_escaped` is set to `True` (default), the validator will allow text containing blocked patterns IF the text is enclosed in the `escape_char` (default `"`).
...
| `allow_escaped`      | Allow blocked patterns if escaped | `True`                  |
