...
class Keyring:
    ...
    def _safe_to_reset_stale_keyring(self) -> bool:
        """Return True when this keyring can be safely reset.

        "Safe" means: the auth_users table either does not exist yet or
        contains no rows, AND/OR the local KEK file (.plexichat_media_key /
        .plexichat_message_key) was created/modified after the keyring file
        itself, which is a strong signal that this is a fresh dev / self-
        test environment and not a real production install. In that case
        the encrypted material has no value to lose and the keyring can be
        regenerated transparently. Production installs (real users in the
        DB AND KEK files older than the keyring) keep the existing FATAL
        behaviour.
        """
        try:
            import os as _os
            ...
            data_dir = _P("~/.plexichat/data").expanduser()
            kek_files = [
                data_dir / ".plexichat_media_key",
                data_dir / ".plexichat_message_key",
            ]
            try:
                keyring_mtime = _os.path.getmtime(self.path)
            except OSError:
                return True
            for kf in kek_files:
                if not kf.exists():
                    continue
                try:
                    if _os.path.getmtime(kf) > keyring_mtime + 1:
                        return True
                except OSError:
                    continue
        except Exception:
    ...
    def load(self):
        ...
        def _load_impl():
            ...
            try:
                ...
                # Safe auto-recovery for the self-test / dev flow: when a
                # keyring cannot be decrypted (typically because a local KEK
                # file like .plexichat_media_key was regenerated by the CLI
                # bootstrap) and the auth_users table has no rows yet, the
                # encrypted data has no value to lose. Wipe the stale file
                # in place and start fresh; production installs with real
                # users keep the existing FATAL behaviour.
                if self._safe_to_reset_stale_keyring():
                    logger.warning(
                        "Keyring %s is unreadable (%s); resetting it because "
                        "the database is empty (dev / self-test path).",
                        self.path.name,
                        e,
                    )
                    try:
                        ...
                        logger.error(
                            "Failed to remove stale keyring %s: %s",
                            self.path,
                            rm_err,
                        )
                    ...
                    self.keys = {}
                    self.rotated_at = 0
                    self.blind_index_root_key = None
                    return
